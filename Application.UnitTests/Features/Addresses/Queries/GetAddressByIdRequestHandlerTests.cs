...
{
    public class GetAddressByIdRequestHandlerTests
    {
        [Fact]
        public async Task GetAddressByIdRequestHandler_AddressExists_ReturnsAddress()
        {
            #region Arrange          
            ...
            #region Act
            var handler = new GetAddressByIdRequestHandler(mockAddressRepository.Object);
            var result = await handler.Handle(new GetAddressByIdRequest() { Id = 1 }, CancellationToken.None);
            #endregion
            ...
            static Mock<IAddressRepo> GetAddressRepository(int id)
            {
                ...
                mockAddressRepository.Setup(r => r.GetByIdAsync(id)).ReturnsAsync(Addresess().Where(a => a.Id == id).FirstOrDefault());
        ...
        [Fact]
        public async Task GetAddressByIdRequestHandler_AddressNotExists_ReturnsResultDataNull()
        {
            #region Arrange          
            ...
            #region Act
            var handler = new GetAddressByIdRequestHandler(mockAddressRepository.Object);
            var result = await handler.Handle(new GetAddressByIdRequest() { Id = 10 }, CancellationToken.None);
            #endregion
